...
def _build_sync_index(concepts: list[ConceptAST]) -> dict:
    ...
    for c in concepts:
        for clause in c.sync:
            ...
            index[key].append({
                "reactor": c.name,
                "local_action": clause.local_action,
                "trigger_params": clause.trigger_params,
                "local_params": clause.local_params,
            })
    return index
    ...
    html += `<div class="detail-section">
      ...
      ${c.sync.map(s => `
        <div class="sync-item" onclick="selectConcept('${s.trigger_concept}')">
          when <span class="concept-link">${s.trigger_concept}</span>.${s.trigger_action}(${s.trigger_params.join(', ')})
          → ${s.local_action}(${s.local_params.join(', ')})
        </div>`).join('')}
    </div>`;
  }
