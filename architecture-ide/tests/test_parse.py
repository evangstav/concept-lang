...
class TestP1Gate:
    ...
    def test_all_positive_fixtures_parse(self):
        concept_files = sorted(f for f in self.FIXTURES_ROOT.rglob("*.concept") if "negative" not in f.parts)
        sync_files = sorted(f for f in self.FIXTURES_ROOT.rglob("*.sync") if "negative" not in f.parts)
